...
export const inject = ['webServer', 'subprocess', 'fs'] as const
...
interface FsFace {
  resolve(path: string, opts?: object): Promise<unknown>
 ...
 */
function restartScript(nodePath: string, binPath: string, cwd: string, logPath: string): string {
  return [
...
export async function apply(ctx: InjectedCtx): Promise<void> {
  ...
  // Write the restarter once at plugin activation.
  const target = await ctx.fs.resolve(restarterFile)
  await ctx.fs.writeText(target, restartScript(nodePath, binPath, cwd, logFile))
  const restarterPath = ctx.fs.processPath(target)

  // The launcher fed to a short-lived, hidden `powershell -Command -`. Flat
  // script (no here-string): WMI-creates the restarter so it runs OUTSIDE the
  // sandbox job and survives dsh web's death.
  const launcher = [
    "$ErrorActionPreference = 'SilentlyContinue'",
    "Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = \"" + PS + ' -NoProfile -WindowStyle Hidden -File `"' + restarterPath + '`"" } | Out-Null',
  ].join('\n')

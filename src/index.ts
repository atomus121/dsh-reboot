 ...
 */
function restartScript(nodePath: string, binPath: string, cwd: string): string {
  return [
 ...
 */
function launcherScript(restart: string): string {
  return [
    "$ErrorActionPreference = 'SilentlyContinue'",
    "$restart = @'",
    restart,
    ...
    'Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = "' + PS + ' -NoProfile -WindowStyle Hidden -File `"$p`"" } | Out-Null',
  ].join('\n')
}
